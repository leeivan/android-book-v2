...
把这个边界分清楚之后，你会更容易回答很多实际问题。例如，“我要加一个依赖，应该改项目级还是模块级脚本”“为什么我改了根目录的某个文件，应用行为却没有变化”“以后如果增加第二个功能模块，它和 `app` 是什么关系”。对于目前这一阶段来说，你至少要知道：绝大多数直接影响应用运行结果的内容，都在 `app` 模块里。
...
### 教材化延伸：为什么项目结构不能只当目录说明来读

Android Studio 和 Android Developers 很适合告诉你 `app/src/main`、`res`、`AndroidManifest.xml`、Gradle 脚本分别放什么，但这类资料更像“目录功能说明”。真正更接近教材的部分，在于你要能同时看见两个层次：一个最小模板工程是如何工作的，一个真实样例项目在规模变大后又是如何避免结构失控的。只有这两层一起看，项目结构才不只是“记住文件夹名字”，而会变成“知道什么职责该落在哪一层”。

### 资料路线

- 先用本章和 first app 练习看清最小工程骨架。
- 再回到 Android Studio 项目结构、资源系统和清单文档，确认目录职责与构建入口。
- 最后对照 `android/architecture-samples` 或 `android/nowinandroid` 观察项目放大后的模块和目录组织。
