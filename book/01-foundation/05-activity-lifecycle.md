...
如果把这些阶段放成一张状态图，读者通常会更容易意识到：生命周期真正描述的不是“方法背诵顺序”，而是页面和系统之间的关系在不断变化。尤其当配置变化和进程恢复一起考虑时，页面实例并不可靠这件事会一下子变得很具体。

![Activity 生命周期状态图](../media/01-foundation/activity-lifecycle-state-map.svg)

图：Activity 生命周期状态图。横向主链描述的是页面从建立、可见、可交互到退到后台的常见过程，右侧则提醒你轻量 UI 状态和长期状态不应由同一套机制托管。
...
1. 概念理解题：为什么 Activity 生命周期讨论的重点不是背回调名字，而是理解“页面实例并不稳定”这件事？
2. 编码实现题：在一个 Activity 中为主要生命周期回调加日志，并验证应用切到后台、旋转屏幕、重新回到前台时日志顺序发生了什么变化。
3. 拓展思考题：哪些状态应该放在 Activity、本地临时状态、`SavedStateHandle` 或 ViewModel 中？你会怎样划分它们的职责？
