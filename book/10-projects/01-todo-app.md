...
这让待办应用成为本地优先最自然的教材案例。Room 不是补充缓存，而是第一可信来源；Repository 不是额外包一层，而是负责统一读写入口；ViewModel 不是多活一会儿的对象，而是把列表页和编辑页状态真正组织起来的地方。
...
这组代码把 Todo App 最值得练的三件事收到了同一条线上。Room 负责本地任务清单，Repository 把“保存任务”和“调度提醒”组合成同一个业务动作，ViewModel 则只把本地任务流翻译成页面状态。只要这三层站稳，Todo 就不会退化成“一个页面直接改数据库，再在按钮点击里再调起提醒”的耦合练习。
