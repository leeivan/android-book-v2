...
Android 官方的数据与文件存储总览，也正是按“应用私有文件、共享存储、偏好配置、本地数据库”这几类来划分，而不是先从工具名字出发。对读者来说，这种划分方式很重要，因为它提醒你：存储方案的第一原则是“边界正确”，而不是“哪种 API 用起来最省事”。
...
- 读者会发现同一个应用往往天然需要多种存储方案。
- 读者会开始用“生命周期和边界”来分类数据，而不是只看 API 熟不熟。
- 读者应能在后续章节里更快判断某份数据应该落到哪一类方案。

自检方式：

- 读者应能解释：为什么“所有数据都塞进同一种存储”通常是坏主意。
- 读者应能区分：一份数据是配置、文件、结构化记录，还是共享媒体/文档访问问题。
- 读者应能判断：某份数据丢失后是“可以重建”，还是会直接破坏用户预期。
...
下一章将先处理最轻量的一类数据，也就是设置项与偏好配置，并把 DataStore 作为现代 Android 的默认主线来展开。
