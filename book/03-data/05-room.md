...
### 教材化延伸：为什么 Room 不能只读成一组注解

Room 官方文档能清楚说明 Entity、DAO、Database 和 Migration 的写法，但教材更需要帮助读者理解这套抽象究竟替你省掉了什么风险。只背注解很容易学成“会写 DAO”，却依然不知道数据流为什么要接 Flow、迁移为什么不能等到上线再补、为什么数据库对象不该直接出现在 UI 层。

### 资料路线

- 先用本章最小实体和 DAO 练习看清 Room 的三层结构。
- 再查官方 Room 文档和 codelab，确认查询、事务、迁移与异步读取的正式用法。
- 最后阅读样例项目中的本地数据层，观察 Room 如何和 Repository、Flow、页面状态结合。
