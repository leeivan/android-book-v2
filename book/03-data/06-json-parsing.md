...
因此，解析层最理想的输出通常不是“把服务器字段原封不动地继续往后传”，而是先在边界处完成一次语义翻译：哪些字段可信，哪些字段需要默认值，哪些字段应组合成更贴近业务语言的模型。这样做的代价是多写一层转换代码，但收益是当接口演进、字段弃用或后端临时返回异常值时，受影响的范围会被控制在数据入口附近。
...
- 读者会开始把 JSON 解析理解成“外部数据进入内部模型”的过程。
- 读者应能区分接口模型和页面模型的不同职责。
- 读者会更早意识到默认值和容错策略的重要性。

自检方式：

- 读者应能解释：为什么 JSON 解析不只是“字符串转对象”。
- 读者应能判断：某个字段是否适合直接暴露给 UI。
- 读者应能说出：默认值和转换逻辑应尽量挡住哪些外部不稳定性。
