...
1. 概念理解题：什么样的信号说明一段逻辑已经超出页面状态组织范围，值得抽成 UseCase / Domain 层？
2. 编码实现题：从一个已有 ViewModel 中抽出一段多步业务流程，定义清晰的输入和输出，并让 ViewModel 只负责调用与映射结果。
3. 拓展思考题：在什么情况下，继续给每个简单调用都包一层 UseCase 会变成过度设计？你会用什么标准及时停手？
