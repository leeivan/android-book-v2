...
### 教材化延伸：为什么 UseCase 和领域层不能写成名词装饰

很多项目会机械地引入 UseCase，却说不清它到底替系统减轻了什么复杂度。教材化写法必须先让读者看到问题场景：当多个页面共享同一业务规则、当一个交互要协调多个仓库、当业务校验不适合塞进 ViewModel 时，UseCase 才会真正显示价值。否则它很容易变成“为了整洁而再加一层”。

### 资料路线

- 先用本章示例判断哪些逻辑只是页面控制，哪些已经上升为可复用业务规则。
- 再对照 Android domain layer recommendations，确认官方对 UseCase 的适用边界。
- 最后在项目案例章节里观察多步骤业务流如何真正通过领域层组合。
