...
把这条关系画出来之后，很多“为什么不直接把网络结果塞给页面”的疑问就会更容易回答。Repository 的价值并不在于挡住所有底层类型，而是在于把多来源数据整理成一条上层可推理、可观察、可回放的出口。

![Repository 多来源数据流示意图](../media/05-architecture/repository-data-flow.svg)

图：Repository 多来源数据流示意图。内存缓存、本地数据库和远程接口都可以同时存在，但页面最好只通过 Repository 观察一条稳定出口；远程刷新也应先写回可信来源，再由可信来源把变化传回上层。
...
1. 概念理解题：Repository、数据源和“单一可信来源”分别在回答什么问题？为什么它们不能混成一层？
2. 编码实现题：选一个同时依赖网络和本地数据库的页面，把读路径改成“统一 Repository -> 可信来源 -> UI 状态”的结构。
3. 拓展思考题：当 Repository 里开始出现页面文案、导航判断或跨多个仓库的大段业务编排时，你会怎样继续拆分边界？
