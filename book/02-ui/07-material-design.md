...
状态反馈如果只停留在颜色变化，也是不够的。更稳妥的 Material 实践，通常会把颜色、文案、图标、动效和可访问性提示一起考虑。例如错误态不应只靠变红来表达，还应有清楚的错误文案；可点击区域不能只看视觉尺寸，还要满足足够的触控目标；对比度、焦点顺序和语义标签也会直接影响页面是否真正可用。换句话说，Material 不是“把界面做得像样”，而是让视觉、一致性和可达性一起工作。
...
- 读者会开始把页面看成有层级和语义的系统，而不是控件堆叠。
- 同类元素会有更统一的表现，页面不再“每块都像不同人做的”。
- 用户动作后的反馈会更清楚，而不是点完按钮只能靠猜。

自检方式：

- 读者应能说出当前页面的主操作是什么，次操作是什么，它们为什么要长得不一样。
- 读者应能判断：某个颜色是在表达品牌角色，还是在表达状态语义。
- 读者应能指出页面里哪些样式应该进入主题或设计令牌，而不是继续散落在控件属性里。
...
下一章将把视角切到现代 Android UI 的另一条主线：Jetpack Compose。到那时，你会看到声明式 UI 为什么天然适合和状态驱动、Material 设计系统一起工作。
