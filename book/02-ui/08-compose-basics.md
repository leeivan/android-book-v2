...
- 读者应能感受到 Compose 的核心不是“控件语法变化”，而是“状态驱动 UI”。
- 读者应能区分局部本地状态、可保存状态和应提升的状态。
- 读者应能看懂：为什么真实项目会把 route 层和 content 层拆开。
- 你开始习惯从“状态和事件”角度理解界面，而不是从“对象和 setter”角度理解界面。

自检方式：

- 读者应能解释：为什么 Compose 是声明式 UI。
- 读者应能说出：`remember` 和 `rememberSaveable` 各自解决什么问题。
- 读者应能判断：某个状态应该留在本地，还是应该被提升或交给 ViewModel。
- 读者应能解释：为什么只刷 Compose 组件 API 仍然可能学不会 Compose。
