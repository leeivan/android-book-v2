...
### 教材化延伸：为什么控件章节不能写成控件清单

API 参考页可以告诉你按钮、输入框、切换控件有哪些属性，但教材更需要解释这些控件分别适合什么交互场景、为什么有些状态该交给组件自己处理、为什么另一些状态必须回到页面状态层。只把控件当成“会摆出来的零件”，读者很快就会在真实表单、校验和状态同步场景里失去判断。

### 资料路线

- 先用本章最小表单和交互练习，把常见控件放进具体场景里使用。
- 再查官方组件文档和 Material 组件说明，确认组件语义、输入类型和错误提示方式。
- 最后阅读一个真实表单或设置页样例，观察控件、状态和校验是怎样配合的。
