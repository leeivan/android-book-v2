...
- 读者应能做出一个最小但语义完整的表单页面。
- 输入区不再只是“两个空白框”，而是带有明确标签和状态反馈。
- 主操作、状态切换和信息展示在页面上职责分明。

自检方式：

- 读者应能为每个控件说出一句话：它在这个页面里承担什么任务。
- 读者应能指出哪个控件负责信息展示，哪个负责输入，哪个负责状态选择，哪个负责主操作。
- 读者应能判断一个控件如果被替换成别的控件，页面语义会不会变差。
...
下一章将继续讨论，当这些控件被用户真正操作时，事件应该如何组织，页面又该如何对这些交互做出稳定反馈。
