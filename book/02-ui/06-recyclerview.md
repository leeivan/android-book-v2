...
### 教材化延伸：为什么 RecyclerView 不能只学成 Adapter 模板

很多读者会把 RecyclerView 学成一套固定模板：写 Adapter、写 ViewHolder、绑定数据。这样能跑，但很难应对真实列表页里最重要的问题，比如差量更新、滚动状态、列表项职责拆分和点击事件上抛。教材化理解需要把列表体验、数据变化和界面复用放到一起讲，而不是只给出一份 Adapter 骨架。

### 资料路线

- 先用本章最小列表示例理解“列表项是什么、谁负责绑定、谁负责点击”。
- 再回看官方 RecyclerView 和 DiffUtil 文档，确认复用与更新机制。
- 最后对照样例项目中的列表页结构，观察列表状态、分页和页面状态如何协作。
