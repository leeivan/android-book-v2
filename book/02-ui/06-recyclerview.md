...
Big Nerd Ranch 在 `CriminalIntent` 里用了一个很适合入门的例子来演示这套分工：先在 `CrimeListViewModel` 里准备 100 条假 `Crime` 数据，再让 `CrimeListFragment` 只负责设置 `LinearLayoutManager` 和 adapter，真正的标题、日期绑定留给 `CrimeListAdapter`。这个案例的价值不在“犯罪列表”这个业务本身，而在于它故意把数据准备、列表容器和条目绑定分开，让读者一眼看清 RecyclerView 自己并不理解 `Crime` 是什么，它只负责向 adapter 索要 ViewHolder 并要求完成绑定。
...
这也是 Big Nerd Ranch 在同一条教学线上继续往前推进的重点：一开始先用固定的 100 条假数据把 `onCreateViewHolder()`、`onBindViewHolder()` 和复用机制讲明白，随后再引到 `ListAdapter + DiffUtil`，说明真实列表不会永远停留在“静态数据 + 整表刷新”。这个推进节奏很值得保留，因为它提醒我们，RecyclerView 的学习终点不是“能显示一列数据”，而是能为后续插入、删除、局部更新和动画过渡预留结构。
...
同样的道理也适用于更新逻辑：`position` 只是当前时刻的排列位置，不是业务身份。列表插入、删除、筛选或排序一发生，position 就可能变化。如果你的点击、勾选或局部刷新完全围绕 position 组织，后面很容易把状态绑错项。真正稳定的锚点，应该是条目的 ID 或能唯一标识该数据的键。
...
- 读者应能清楚区分 Adapter、ViewHolder 和 LayoutManager 的职责。
- 列表项结构不会因为加一个状态或操作入口就迅速变乱。
- 你开始建立“列表更新应尽量表达差异”的意识。

自检方式：

- 读者应能说出：为什么 RecyclerView 不只是“显示很多项”。
- 读者应能解释：为什么列表点击事件不该围绕某个临时视图对象组织。
- 读者应能判断：当前页面卡顿是 RecyclerView 自身问题，还是条目设计与绑定策略的问题。
...
下一章将把视角从单个列表和控件继续拉高，进入 Material Design。到那时，你会开始把页面不再只看成“功能容器”，而是看成有层级、反馈和一致性要求的设计系统。
