...
### 教材化延伸：为什么布局基础不能只看属性表

布局文档通常擅长解释某个容器或属性怎么写，但初学者真正缺的往往不是“再多记几个属性”，而是版面组织的直觉。教材化学习更需要把属性说明、最小布局练习和响应式界面样例结合起来。只有亲手做过几次“改约束、看结果、再调布局”的练习，`ConstraintLayout`、间距、对齐和层级关系才会从规则条目变成空间直觉。

### 资料路线

- 先用本章的最小布局练习建立盒子、约束和层级关系的直觉。
- 再回看官方布局与自适应设计文档，确认不同屏幕和容器的正式规则。
- 最后对照 Compose 布局 codelab 或官方样例，理解现代项目如何把布局规则转成更稳定的界面结构。
