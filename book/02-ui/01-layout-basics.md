...
- 读者应能明显感受到“先分区再放控件”比“先堆控件再补结构”更稳定。
- 读者应能比较出简单线性布局和约束布局在扩展性上的差异。
- 页面结构即使增加一个空状态，也不会立刻变乱。

自检方式：

- 读者应能说清哪个容器在承担结构组织，哪个控件在承担内容展示。
- 读者应能指出当前页面里哪些层级是真有必要，哪些只是为了图省事加出来的。
- 读者应能回答：为什么某些尺寸应该放资源里，而不是直接写常量。
...
下一章将在这个结构基础上继续往前走，讨论常用控件在真实页面任务里分别承担什么角色，以及为什么控件选择本质上是交互语义问题。
