...
- 读者应能把“网络发生了什么”和“页面显示什么”分成两层处理。
- 读者应能明显减少 ViewModel 里重复的 `try-catch` 和空值判断。
- 读者会更容易在后续章节里加入缓存、离线优先或统一重试策略。

自检方式：

- 读者应能解释：协议错误、业务错误和解析错误为什么不是同一类问题。
- 读者应能判断：哪些信息应保留在 `ApiResult` 里，哪些应留给 UI 再决定。
- 读者应能确认：UI 已经不再直接依赖 `HttpException`、`IOException` 或裸响应对象。
- 读者应能检查：同一类错误在不同页面是否至少有一致的处理起点。
