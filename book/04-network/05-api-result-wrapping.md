...
### 教材化延伸：为什么结果包装不是“多包一层类”

很多读者初看 `ApiResult`、`Result<T>` 或密封类包装，会觉得这只是“让代码更复杂”。官方文档通常会解释状态持有者和密封类本身，但教材更需要说明：这层包装真正解决的是协议结果、业务结果和页面结果之间的翻译问题。没有这层翻译，UI 往往只能直接面对异常、空值和 DTO。

### 资料路线

- 先用本章的密封类结果模型理解“网络成功”与“页面可显示”并不是同一件事。
- 再对照 Android UI state 与 Kotlin sealed classes 文档，确认状态建模方式。
- 最后阅读样例项目或下一章缓存章节，观察统一结果模型如何继续被数据层复用。
