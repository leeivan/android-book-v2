...
### 9. 学 Retrofit 时，把协议层、语言层和工程层分开看

Retrofit 文档最擅长确认注解、`baseUrl`、转换器和返回语义；Kotlin 与序列化资料解释 `suspend`、DTO 和 JSON 转换为何能自然衔接；样例项目才真正展示 Retrofit 在 Repository、离线缓存和 `uiState` 里的站位。把这三层分开看，Retrofit 就不会退化成“会写注解”的技巧章，而会重新回到接口契约与工程边界。
...
- 读者应能把“接口声明”和“底层通信”明确分层。
- 读者应能用更稳定的接口契约描述网络能力。
- 读者应能区分：Retrofit 文档、Kotlin/序列化文档和样例项目分别解决什么问题。
- 读者会更容易在下一章里做统一结果包装和错误处理。

自检方式：

- 读者应能解释：Retrofit 和 OkHttp 分别解决什么问题。
- 读者应能判断：什么时候接口适合直接返回 body，什么时候需要 `Response<T>`。
- 读者应能说明：为什么 DTO 不应直接一路传到 UI。
- 读者应能确认：统一头信息是否已经回到 OkHttp 层，而不是散落在 Retrofit 方法签名里。
- 读者应能解释：为什么只会写 Retrofit 注解，还不等于已经会设计接口层。
