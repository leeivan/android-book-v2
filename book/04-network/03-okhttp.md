...
OkHttp 是现代 Android 网络栈里非常重要的一层基础设施。它不是为了让你在每个项目里都手写原始 HTTP 请求，而是为了让你知道，真正负责“把请求可靠地发出去”的组件，应该承担哪些职责。放到更完整的工程结构里看，OkHttp 往往应该沉在网络模块和依赖注入层里，被 Retrofit 和数据层统一复用，而不是直接长到页面代码里。只要这层职责边界理解清楚，Retrofit、Repository 和统一错误处理才不容易互相污染。
