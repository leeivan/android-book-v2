...
### 教材化延伸：为什么 OkHttp 不能只学成发请求工具

如果只看 `Request.Builder()` 和同步异步调用，OkHttp 很容易被误解成“手写 HTTP 的工具类”。但真正更像教材的理解，是把它看成网络基础设施：连接池、超时、重试、TLS、拦截器和统一横切能力都落在这层。只有把这些职责放回客户端层，后面的 Retrofit 和 Repository 分层才不会混乱。

### 资料路线

- 先用本章最小示例看清 `OkHttpClient`、`Call`、`Response` 各自承担什么角色。
- 再阅读官方 Calls、Interceptors 和 Recipes 文档，确认请求执行与扩展点边界。
- 最后回到 Retrofit 章节，观察为什么现代项目总是复用共享客户端而不是在接口层重复配置。
