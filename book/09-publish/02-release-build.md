...
因此 release 构建不能只停在“打包成功”。真正稳定的发布链路需要把候选产物、验证记录和排障材料一起组织起来。只要归档规则没有显式表达，mapping 文件和候选产物迟早会散落到不同机器和不同目录里，到了线上问题出现时再追，就会非常被动。
...
- 读者会把 release 当成独立的正式产物，而不是 debug 的导出模式。
- 读者应能清楚区分 AAB 和 APK 在发布链路中的角色。
- 读者会为线上问题预先准备最低限度的定位材料。

自检方式：

- 读者应能解释为什么 debug 能跑不代表 release 安全。
- 读者应能说明 `bundleRelease` 和 `assembleRelease` 分别更适合什么场景。
- 读者应能回答“当前版本出线上崩溃时，团队靠什么把堆栈还原回来”。
