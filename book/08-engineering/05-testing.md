...
- 读者会从风险和收益角度而不是从工具角度看测试。
- 读者应能更清楚地区分哪些测试该写在网络层、状态层和 UI 层。
- 读者会更明显感受到边界清晰的代码为什么更容易被测。

自检方式：

- 读者应能说明为什么固定输入和可控环境对测试稳定性很关键。
- 读者应能解释一条好的 ViewModel 测试到底在保护什么状态变化。
- 读者应能判断某个 UI 测试是否真的对应高价值主路径。
